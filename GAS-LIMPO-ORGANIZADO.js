...
function doGet(e) {
  ...
  if (type === "wa_webhook_verify") {
    const mode = String(p['hub.mode'] || p.mode || '');
    const token = String(p['hub.verify_token'] || p.verify_token || '');
    const challenge = String(p['hub.challenge'] || p.challenge || '');
  ...
  if (type === "get_status") {
    var slugStatus = normalizeSlug_(String(p.site || ""));
    return jsonOut_(getStatus_(ss, slugStatus));
  }
  if (type === "get_site_sections") {
    var slugSections = normalizeSlug_(String(p.site || ""));
    return getSiteSections_(ss, { site: slugSections });
  }
  if (type === "validate_vip_pin") {
    var slugValidate = normalizeSlug_(String(p.site || ""));
    var pin = String(p.pin || "");
    return jsonOut_(validateVipPinAPI_(ss, slugValidate, pin));
  }
  ...
  if (type === "get_analytics") {
    var slugAn = normalizeSlug_(String(p.site || ""));
    var rng    = String(p.range || "30d");
    return jsonOut_(getAnalytics_(slugAn, rng));
  }
...
function doPost(e) {
  ...
  try {
    ...
    if (data.action === 'auth_status') {
      ...
      const site = String(data.site || data.siteSlug || "").trim();
      const pin = String(data.pin || data.vipPin || "").trim();
      const email = String(data.email || "").trim().toLowerCase();
      return jsonOut_(get_auth_status(site, pin, email));
    }
    ...
    if (type === "update_site_content") {
      return updateSiteContent_(ss, data);
    }
    if (type === "record_hit") {
...
if (type === 'wa_send' || type === 'wa_send_text') {
  try {
    ...
    if (code >= 200 && code < 300) {
      // SEMPRE salvar mensagem enviada na planilha
      try {
        const shWA = ensureWhatsAppSheet_(ss);
        const messageId = json.messages?.[0]?.id || 'unknown_' + Date.now();
        const now = new Date();
        
        // ESTRUTURA CORRETA: timestamp, siteSlug, phone_id, from, to_display, msg_id, type, text, raw_json
        shWA.appendRow([
          now,                    // timestamp
          site,                   // siteSlug
          phoneId,                // phone_id (ID do telefone do negócio)
          '',                     // from (vazio para mensagens enviadas)
          to,                     // to_display (número de destino)
          messageId,              // msg_id
          'text',                 // type
          text,                   // text
        ...
        log_(ss, 'wa_message_stored', { site, messageId, direction: 'out', to: to });
      } catch (storeError) {
        log_(ss, 'wa_message_store_error', { site, error: String(storeError) });
        // NÃO falhar o envio se não conseguir salvar
      }
      
      return jsonOut_({ ok: true, result: json });
    }
...
if (type === 'wa_list_contacts') {
  try {
    ...
    // Cache para contatos (5 minutos)
    const cacheKey = `wa_contacts::${site}::${page}::${pageSize}`;
    const cached = getCachedData_(ss, cacheKey);
    if (cached) {
      log_(ss, "wa_list_contacts_cache_hit", { site, page, pageSize });
      return jsonOut_(cached);
    }

    // Primeiro tentar listar da planilha de contatos
    let contacts = listWhatsAppContacts_(ss, site, page, pageSize, data.filters || {});
    
    // Se não há contatos, criar baseado nas mensagens
    if (!contacts.items || contacts.items.length === 0) {
      contacts = createContactsFromMessagesList_(ss, site);
    }
    ...
    log_(ss, 'wa_list_contacts', { 
      ...
      returned: contacts.contacts ? contacts.contacts.length : contacts.items.length 
    });
    
    const result = { 
      ok: true, 
      contacts: contacts.contacts || contacts.items,
      total: contacts.total,
      page: contacts.page,
    ...
    if (result.ok) {
      setCachedData_(ss, cacheKey, result, 5); // 5 minutos
    }
...
// ===== FUNÇÃO: google_reviews =====
// Busca reviews do Google My Business
if (type === 'google_reviews') {
  try {
    const site = normalizeSlug_(String(data.site || data.siteSlug || ''));
    const action = String(data.action || 'get_reviews');
    
    if (action === 'get_reviews') {
      return jsonOut_(getGoogleReviews_(ss, site));
    } else if (action === 'sync_reviews') {
      return jsonOut_(syncGoogleReviews_(ss, site));
    } else if (action === 'setup_oauth') {
      return jsonOut_(setupGoogleOAuth_(ss, site, data));
    } else if (action === 'save_credentials') {
      return jsonOut_(saveGoogleCredentials_(ss, site, data));
    } else {
...
if (type === 'feedback') {
  try {
    ...
    if (action === 'submit') {
      return jsonOut_(submitFeedback_(ss, site, data));
    } else if (action === 'list') {
      return jsonOut_(listFeedbacks_(ss, site, data));
    } else if (action === 'approve') {
      return jsonOut_(approveFeedback_(ss, site, data));
    } else if (action === 'reject') {
      return jsonOut_(rejectFeedback_(ss, site, data));
    } else if (action === 'get_public') {
      return jsonOut_(getPublicFeedbacks_(ss, site, data));
    } else {
      return jsonOut_({ ok: false, error: 'Ação inválida' });
    }
  } catch (err) {
    ...
    if (type === "recordEvent_") {
      return recordEvent_(ss, data);
    }
    ...
    if (type === "whatsapp_webhook" || type === "wa_incoming") {
      ...
      msgs.forEach(function(m){
        // ESTRUTURA CORRETA para mensagens recebidas
        const rawData = {
          direction: 'in',
          received: true,
          messageId: m.id,
        ...
        shWA.appendRow([
          ...
          siteSlug,                      // siteSlug
          phoneId||'',                   // phone_id (ID do telefone do negócio)
          (m.from||''),                  // from (número de quem enviou)
          (change.metadata && change.metadata.display_phone_number)||'', // to_display (número do negócio)
          m.id||'',                      // msg_id
          m.type||'text',                // type
          (m.text && m.text.body) ? m.text.body : (m.button && m.button.text) ? m.button.text : '', // text
          JSON.stringify(rawData),       // raw_json
          (m.status||'')                 // status
        ]);
      });
    ...
    if (type === "wa_list_messages") {
      ...
      // Cache para mensagens (2 minutos)
      var cacheKey = `wa_messages::${siteList}::${page}::${pageSize}`;
      var cached = getCachedData_(ss, cacheKey);
      if (cached) {
        log_(ss, "wa_list_messages_cache_hit", { site: siteList, page, pageSize });
        return jsonOut_(cached);
      }
      
      var out = listWhatsAppMessages_(ss, siteList, page, pageSize);
      
      // Salvar no cache se sucesso
      if (out.ok) {
        setCachedData_(ss, cacheKey, out, 2); // 2 minutos
      }
...
/** Mapeia telefones para nomes de contatos */
function getContactsMap_(ss, siteSlug) {
  try {
    const sh = ss.getSheetByName('whatsapp_contacts');
    if (!sh) {
      // Se não há planilha de contatos, criar contatos baseados nas mensagens
      return createContactsFromMessages_(ss, siteSlug);
    }
    ...
    const idx = {
      ...
      phone: hdr.indexOf('telefone_normalizado'),
      name: hdr.indexOf('nome')
    };
    
    const map = {};
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      const rowSite = String(row[idx.site] || '');
      const phone = String(row[idx.phone] || '');
      const name = String(row[idx.name] || '');
      
      if (rowSite === siteSlug && phone && name) {
        map[phone] = name;
      }
    }
    
    // Se não encontrou contatos na planilha, criar baseado nas mensagens
...
/** Cria contatos baseados nas mensagens existentes */
function createContactsFromMessages_(ss, siteSlug) {
  try {
    ...
    const contactsMap = {};
    const phoneToContact = {}; // Mapear telefone para nome do contato
    
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      const rowSite = String(row[idx.site] || '');
      
      if (rowSite !== siteSlug) continue;
      
      const fromPhone = String(row[idx.from] || '');
      const toPhone = String(row[idx.to] || '');
      
      // Para mensagens recebidas: usar o telefone de origem
      if (fromPhone && fromPhone.trim() !== '') {
        const normalizedFrom = normalizePhone_(fromPhone);
        if (!contactsMap[normalizedFrom]) {
          // Usar "Contato" em vez do número formatado
          contactsMap[normalizedFrom] = "Contato";
          phoneToContact[normalizedFrom] = "Contato";
        }
      }
      
      // Para mensagens enviadas: usar o telefone de destino
      if (toPhone && toPhone.trim() !== '') {
        const normalizedTo = normalizePhone_(toPhone);
        if (!contactsMap[normalizedTo]) {
          // Usar "Contato" em vez do número formatado
          contactsMap[normalizedTo] = "Contato";
          phoneToContact[normalizedTo] = "Contato";
        }
      }
    
    // Verificar se há números duplicados (com e sem 9) e consolidar
    const allPhones = Object.keys(contactsMap);
    for (let i = 0; i < allPhones.length; i++) {
      for (let j = i + 1; j < allPhones.length; j++) {
        const phone1 = allPhones[i];
        const phone2 = allPhones[j];
        
        // Se são o mesmo número normalizado, consolidar
        if (normalizePhone_(phone1) === normalizePhone_(phone2)) {
          const name1 = contactsMap[phone1];
          const name2 = contactsMap[phone2];
          
          // Manter o nome melhor (não "Contato")
          if (name1 === "Contato" && name2 !== "Contato") {
            contactsMap[phone1] = name2;
            phoneToContact[phone1] = name2;
          } else if (name2 === "Contato" && name1 !== "Contato") {
            contactsMap[phone2] = name1;
            phoneToContact[phone2] = name1;
          }
        }
      }
    }
...
/** Normaliza telefone brasileiro para formato consistente */
 ...
 * Mesmo raciocínio do front.
 */
function toE164CellBR_(raw) {
  var d = onlyDigits_(raw);
  if (d.indexOf("55") === 0) d = d.slice(2);

  if (d.length >= 11) {
    var n11 = d.slice(-11);
    var ddd = n11.slice(0, 2);
    var rest = n11.slice(2); // 9 dígitos
    if (rest.length !== 9) return "";
    if (rest[0] !== "9") rest = "9" + rest.slice(0, 8);
    return "55" + ddd + rest;
  }

  if (d.length === 10) {
    var ddd10 = d.slice(0, 2);
    var line8 = d.slice(2);
    return "55" + ddd10 + "9" + line8;
  }
...
function normalizePhone_(phone) {
  return toE164CellBR_(phone);
}

/**
 * Consolida contatos duplicados baseado na normalização E.164
 */
function consolidateContacts_(rows) {
  // rows: [{phone: "...", name: "...", ...}, ...]
  var map = {};
  for (var i = 0; i < rows.length; i++) {
    var r = rows[i] || {};
    var key = toE164CellBR_(r.phone || r.msisdn || r.from || "");
    if (!key) continue;
    ...
    var prev = map[key];
    var currName = String(r.name || "").trim();
    var prevName = String(prev.name || "").trim();
    
    // Função para verificar se é um nome real (não número formatado)
    var isRealName = function (s) { 
    ...
    for (var k in r) if (r.hasOwnProperty(k) && k !== "phone") prev[k] = prev[k] || r[k];
    map[key] = prev;
  }
  // retorno como array
...
/**
 * Salva credenciais do Google My Business no settings_kv
 */
function gmbSaveCredentials_(ss, site, email, tokens) {
  try {
    if (!site || !email || !tokens) {
      return { ok: false, error: "Site, email e tokens são obrigatórios" };
    }

    // Validar estrutura dos tokens
    if (!tokens.access_token || !tokens.refresh_token || !tokens.expires_in) {
      return { ok: false, error: "Estrutura de tokens inválida" };
    }
    ...
    // Validar site
    if (site.length < 2 || site.includes(' ')) {
...
function gmbGetReviews_(ss, site, email) {
  try {
    ...
    const cacheKey = `gmb_cache::${site}::${email}`;
    const cached = getCachedData_(ss, cacheKey);
    if (cached) {
      log_(ss, "gmb_get_reviews_cache_hit", { site, email });
    ...
    if (!credentials || !credentials.access_token) {
      return { ok: false, error: "Credenciais não encontradas. Conecte sua conta Google primeiro." };
    }

    // Descriptografar tokens
    try {
      if (credentials.salt) {
        // Nova versão com criptografia
        const salt = credentials.salt;
        const decryptedAccess = Utilities.base64Decode(credentials.access_token).replace(salt, '');
        const decryptedRefresh = Utilities.base64Decode(credentials.refresh_token).replace(salt, '');
        
        credentials.access_token = decryptedAccess;
        credentials.refresh_token = decryptedRefresh;
      }
      // Se não tem salt, usar tokens diretos (compatibilidade com versão antiga)
    } catch (e) {
      log_(ss, "gmb_decrypt_error", { site, email, error: String(e) });
      return { ok: false, error: "Erro ao descriptografar credenciais" };
    }
    ...
    if (now >= expiresAt) {
      ...
      if (refreshed.ok) {
        ...
        const encryptedAccess = Utilities.base64Encode(refreshed.access_token + salt);
        ...
        // Atualizar no settings_kv
        for (let i = 1; i < data.length; i++) {
          if (data[i][0] === key) {
            settingsSheet.getRange(i + 1, 2).setValue(JSON.stringify(newCredentials));
            break;
          }
        }
        
        // Usar token descriptografado para a chamada da API
        credentials.access_token = refreshed.access_token;
      } else {
        return { ok: false, error: "Token expirado e não foi possível renovar. Reconecte sua conta Google." };
      }
    }
...
async function refreshGoogleToken_(refreshToken) {
  try {
    ...
    const clientId = props.getProperty('GMB_CLIENT_ID');
    const clientSecret = props.getProperty('GMB_CLIENT_SECRET');
    ...
    if (response.getResponseCode() === 200) {
      ...
      return { ok: false, error: result.error || 'Erro ao renovar token' };
    }
  } catch (e) {
    return { ok: false, error: String(e) };
  }
...
async function getGoogleBusinessInfo_(accessToken) {
  try {
    ...
    if (accountsResponse.getResponseCode() !== 200) {
      return { ok: false, error: 'Erro ao buscar contas do Google My Business' };
    }

    const accounts = JSON.parse(accountsResponse.getContentText());
    if (!accounts.accounts || accounts.accounts.length === 0) {
      return { ok: false, error: 'Nenhuma conta do Google My Business encontrada' };
    }
    ...
    if (locationsResponse.getResponseCode() !== 200) {
      return { ok: false, error: 'Erro ao buscar locais do Google My Business' };
    }

    const locations = JSON.parse(locationsResponse.getContentText());
    if (!locations.locations || locations.locations.length === 0) {
      return { ok: false, error: 'Nenhum local do Google My Business encontrado' };
    }

    const location = locations.locations[0];
    return {
      ok: true,
      placeId: location.name.split('/').pop(),
      businessInfo: {
        name: location.title || 'Negócio',
        address: location.storefrontAddress?.addressLines?.join(', ') || 'Endereço não disponível',
        placeId: location.name.split('/').pop()
      }
    };
  } catch (e) {
...
async function getGoogleReviews_(accessToken, placeId) {
  try {
    ...
    if (response.getResponseCode() !== 200) {
      return { ok: false, error: 'Erro ao buscar reviews do Google Places' };
    }

    const data = JSON.parse(response.getContentText());
    if (data.status !== 'OK') {
      return { ok: false, error: data.error_message || 'Erro na API do Google Places' };
    }
    ...
    const reviews = (result.reviews || []).map(review => ({
      ...
      relativeTimeDescription: review.relative_time_description || 'Há algum tempo'
    }));
...
/**
 * Desconecta conta Google My Business
 */
function gmbDisconnect_(ss, site, email) {
  try {
    if (!site || !email) {
      return { ok: false, error: "Site e email são obrigatórios" };
    }
    ...
    log_(ss, "gmb_disconnect", { site, email, removed });

    return { ok: true, message: "Conta Google desconectada com sucesso" };
  } catch (e) {
    return { ok: false, error: String(e) };
  }
}
...
/**
 * Funções de cache para otimizar performance
 */
function getCachedData_(ss, key) {
  try {
    const settingsSheet = ss.getSheetByName('settings_kv');
...
function setCachedData_(ss, key, data, minutes = 30) {
  try {
    const settingsSheet = ss.getSheetByName('settings_kv');
...
function fmtPhoneBR_(phone) {
  // Primeiro normalizar, depois formatar
  const normalized = normalizePhone_(phone);
  const clean = normalized.replace(/\D/g, '');
  
  // Remover o 55 do início para formatação
  const withoutCountry = clean.slice(2);
  
  // Sempre formatar como celular (11 dígitos) após normalização
  if (withoutCountry.length === 11) {
    // Celular: (XX) 9XXXX-XXXX
    return withoutCountry.replace(/(\d{2})(\d{5})(\d{4})/, '($1) $2-$3');
  } else if (withoutCountry.length === 10) {
    // Fixo: (XX) XXXX-XXXX (raro após normalização)
    return withoutCountry.replace(/(\d{2})(\d{4})(\d{4})/, '($1) $2-$3');
  }
  
  // Fallback: retornar o telefone original se não conseguir formatar
  return phone || "Contato";
}

/** Cria lista de contatos baseada nas mensagens */
function createContactsFromMessagesList_(ss, siteSlug) {
  try {
    const contactsMap = createContactsFromMessages_(ss, siteSlug);
    const contacts = [];
    
    for (const [phone, name] of Object.entries(contactsMap)) {
      // Se não tem nome real, usar número formatado como fallback
      const displayName = (name && name !== "Contato") ? name : fmtPhoneBR_(phone);
      
      contacts.push({
        phone: phone,
        name: displayName,
        id: 'msg_' + phone.replace(/\D/g, ''),
        siteSlug: siteSlug,
        ativo: true,
    ...
    const consolidatedContacts = consolidateContacts_(contacts);
    
    return {
      contacts: consolidatedContacts,
      total: consolidatedContacts.length,
      page: 1,
      pageSize: consolidatedContacts.length
    };
  } catch (e) {
    return {
      contacts: [],
...
function listWhatsAppMessages_(ss, siteSlug, page, pageSize) {
  ...
  // Buscar contatos para mapear nomes
  const contactsMap = getContactsMap_(ss, siteSlug);
  ...
  for (var i=1;i<data.length;i++){
    ...
    const fromPhone = String(row[idx.from] || '');
    const toPhone = String(row[idx.to] || '');
    const rawJson = row[idx.raw] ? (typeof row[idx.raw] === 'string' ? JSON.parse(row[idx.raw]) : row[idx.raw]) : {};
    
    // LÓGICA CORRIGIDA: Determinar o telefone do contato
    let contactPhone = '';
    let isReceived = false;
    
    if (fromPhone && fromPhone.trim() !== '') {
      // Mensagem recebida: fromPhone é o cliente
      contactPhone = fromPhone;
      isReceived = true;
    } else if (toPhone && toPhone.trim() !== '') {
      // Mensagem enviada: toPhone é o cliente
      contactPhone = toPhone;
      isReceived = false;
    }
    
    const normalizedContactPhone = normalizePhone_(contactPhone);
    // Priorizar nome real do contato, usar número formatado apenas como fallback
    const contactName = contactsMap[normalizedContactPhone] || contactsMap[contactPhone] || "Contato";
    
    // Garantir que sempre use o número normalizado para evitar duplicatas
    const finalPhone = normalizedContactPhone;
    ...
    all.push({
      ...
      type: row[idx.type],
      text: row[idx.text],
      raw_json: row[idx.raw],
      name: contactName,
      contactName: contactName,
      phoneNumber: finalPhone, // Sempre usar telefone normalizado
      contactPhone: contactPhone, // Telefone original para referência
      isReceived: isReceived, // Flag para identificar se é recebida
      direction: isReceived ? 'in' : 'out' // Direção clara
    });
  }
...
// Função listWhatsappMessages_ removida - use listWhatsAppMessages_() que é a versão otimizada
...
/**
 * 🔐 FUNÇÃO OTIMIZADA - Autentica usuário com email e senha
...
function userLogin_(ss, data) {
  try {
    ...
    const sh = ensureUsuariosSheet_(ss);
    const map = headerIndexMap_(sh);
    const rowIdx = findUserRowByEmail_(sh, email);
    
    if (rowIdx === -1) {
      return jsonOut_({ ok: false, error: 'user_not_found' });
    }

    const actualRow = rowIdx + 2;
    const storedHash = String(sh.getRange(actualRow, map.password_hash + 1).getValue() || '');
    const salt = String(sh.getRange(actualRow, map.salt + 1).getValue() || '');
    
    if (!storedHash) {
      return jsonOut_({ ok: false, error: 'no_password_set' });
    }

    const computedHash = sha256Hex_(pwd + salt);
    ...
    console.error("Erro em userLogin_:", e);
    return jsonOut_({ ok: false, error: String(e) });
  }
}
...
/**
 * 📱 FUNÇÃO OTIMIZADA - Envia mensagem de texto via WhatsApp Business API
 * @param {Object} ss - Spreadsheet
 * @param {string} siteSlug - Slug do site
 * @param {string} to - Número de destino (formato E.164)
 * @param {string} body - Conteúdo da mensagem
 * @returns {Object} { ok: boolean, messageId?: string, error?: string }
 */
function waSendText_(ss, siteSlug, to, body) {
  try {
    if (!siteSlug || !to || !body) {
      throw new Error('Site, destinatário e mensagem são obrigatórios');
    }

    const kv = getKV_(ss, siteSlug);
    const st = safeParseJson_(kv.settings_json || '{}');
    const W = st.whatsapp || {};
    ...
    if (!phoneId || !appToken) {
      throw new Error('missing_phone_or_token');
    }
    ...
    if (code < 200 || code >= 300) {
      throw new Error('wa_send_fail ' + code);
    }
    
    // Armazena a mensagem enviada na planilha
    try {
      const shWA = ensureWhatsAppSheet_(ss);
      ...
      shWA.appendRow([
        now,                    // timestamp
        siteSlug,              // siteSlug
        '',                    // phone_id (vazio para mensagens enviadas)
        '',                    // from (vazio para mensagens enviadas)
        to,                    // to_display
        messageId,             // msg_id
        'text',                // type
      ...
      log_(ss, 'wa_message_stored', { siteSlug, messageId, direction: 'out' });
    } catch (storeError) {
      console.error("Erro ao armazenar mensagem enviada:", storeError);
      // Não falha o envio se não conseguir armazenar
    }
    
    return { ok: true, messageId: json.messages?.[0]?.id || 'unknown' };
  } catch (e) {
    console.error("Erro em waSendText_:", e);
    return { ok: false, error: String(e) };
  }
}
...
/**
 * 👤 FUNÇÃO OTIMIZADA - Retorna dados do usuário autenticado
 * @param {Object} ss - Spreadsheet
...
function userMe_(ss, data) {
  try {
    ...
    const filled = ensureUserFieldsFromCadastros_(ss, sh, map, actualRow, email);

    const siteSlug = String(filled.siteSlug || sh.getRange(actualRow, map.siteSlug + 1).getValue() || '');
    const role = String(sh.getRange(actualRow, map.role + 1).getValue() || 'client');
    const plan = String(filled.plan || sh.getRange(actualRow, map.plan + 1).getValue() || 'essential');
    const lastLogin = String(sh.getRange(actualRow, map.last_login + 1).getValue() || '');
    ...
    console.error("Erro em userMe_:", e);
    return jsonOut_({ ok: false, error: String(e) });
  }
}
 ...
 */
/**
 * 🎯 FUNÇÃO OTIMIZADA - Retorna o plano do cliente (VIP/Essential)
 * @param {string} siteSlug - Slug do site
 * @param {string} emailLC - Email do cliente (lowercase)
 * @returns {Object} { ok: boolean, plan: string, error?: string }
 */
function get_client_plan_v2(siteSlug, emailLC) {
  try {
    ...
    // Se não há dados, retorna essential como padrão
    if (!data || data.length < 2) {
      return { ok: true, plan: "essential" };
    }

    const head = data[0].map(h => String(h).trim());
    const idxEmail = head.indexOf("email");
    const idxSite  = head.indexOf("siteSlug");
    const idxPlanA = head.indexOf("plan");
    const idxPlanB = head.indexOf("plano");

    // Usa 'plan' se existir, senão 'plano' como fallback
    const idxPlan = (idxPlanA !== -1 ? idxPlanA : idxPlanB);
    if (idxPlan === -1) {
      return { ok: false, error: "missing_plan_header" };
    }
    ...
    console.error("Erro em get_client_plan_v2:", e);
    return { ok: false, error: String(e) };
  }
}
 ...
 */
/**
 * 🔐 FUNÇÃO OTIMIZADA - Valida PIN VIP do cliente
 * @param {string} site - Slug do site
 * @param {string} pin - PIN VIP fornecido
 * @returns {Object} { ok: boolean, valid: boolean, error?: string }
 */
function validate_vip_pin(site, pin) {
  try {
    if (!pin || !site) {
      return { ok: false, valid: false, error: "Site e PIN são obrigatórios" };
    }

    const ss = openSS_();
    const normalizedSite = normalizeSlug_(site);
    
    // 1. Primeiro tenta buscar na planilha usuarios (método antigo)
    try {
      ...
      if (usuariosSheet) {
        ...
        const planoIdx = headers.indexOf("plan");

        if (siteIdx !== -1) {
          const data = usuariosSheet.getRange(2, 1, usuariosSheet.getLastRow() - 1, usuariosSheet.getLastColumn()).getValues();
          
          for (let i = 0; i < data.length; i++) {
            const rowSite = normalizeSlug_(String(data[i][siteIdx] || ""));
            
            if (rowSite === normalizedSite) {
      ...
      console.log("Erro ao buscar na planilha usuarios:", e);
    }

    // 2. Se não encontrou, busca na planilha settings_kv (método novo)
    try {
      const settingsSheet = ss.getSheetByName("settings_kv");
      if (settingsSheet) {
        const last = settingsSheet.getLastRow();
        if (last >= 2) {
          const vals = settingsSheet.getRange(2, 1, last - 1, 3).getValues();
          
          for (let i = vals.length - 1; i >= 0; i--) {
            const r = vals[i];
            if (String(r[0] || "").trim().toUpperCase() === normalizedSite) {
              const json = String(r[2] || "{}");
              let obj = {};
              try { 
                obj = JSON.parse(json); 
              } catch (e) {
              ...
              // Busca o PIN nas configurações
              const storedPin = String((obj.security && obj.security.vipPin) || (obj.vipPin) || "").trim();
              const isVip = String(obj.plan || "").toLowerCase().includes("vip") || 
                           String(obj.security && obj.security.plan || "").toLowerCase().includes("vip");
      ...
      console.log("Erro ao buscar na planilha settings_kv:", e);
    }
...
/**
 * @deprecated Use get_client_plan_v2 instead - this function is kept for backward compatibility
...
function get_client_plan(site, email) {
  ...
  return get_client_plan_v2(site, email);
}

/**
 * 🔐 FUNÇÃO OTIMIZADA - Consolida status do site + plano e valida PIN VIP
 * @param {string} site - Slug do site
 * @param {string} pin - PIN VIP (opcional)
 * @param {string} email - Email do cliente (opcional)
 * @returns {Object} { ok: boolean, status: string, active: boolean, plan: string, error?: string }
 */
function get_auth_status(site, pin, email) {
  try {
    ...
    // Busca status do site
    const s = getStatusForSite_(slug);
    if (!s || !s.ok) {
      return { 
        ok: false, 
        error: (s && s.error) ? String(s.error) : "status_unavailable" 
      };
    }

    // Busca plano padrão da aba usuarios
    const p = get_client_plan_v2(slug, email);
    let plan = (p && p.ok) ? p.plan : "essential";

    // Se PIN fornecido, valida e força VIP se válido
    if (pin && pin.trim()) {
    ...
    // Busca dados de cobrança para completar a resposta
    let lastPayment = null;
    let nextCharge = null;
    
    try {
      const billing = get_client_billing_data_(slug, email);
      if (billing && billing.ok) {
        lastPayment = billing.lastPayment;
        nextCharge = billing.nextCharge || billing.nextPayment;
      }
    } catch (e) {
      console.log("Erro ao buscar dados de cobrança:", e);
    }
    ...
    console.error("Erro em get_auth_status:", e);
    return { ok: false, error: String(e) };
  }
}

/**
 * Busca dados de cobrança do cliente
 */
function get_client_billing_data_(site, email) {
  try {
    const ss = openSS_();
    const emailLC = email ? email.toLowerCase() : "";
    
    if (!emailLC) return { ok: false, error: "missing_email" };
    ...
    try {
      ...
      if (idxEmail === -1) return { ok: false, error: "missing_email_header" };
        ...
        if (shPagamentos) {
          ...
          if (idxSitePag !== -1 && idxDate !== -1) {
            ...
            if (pagamentos.length > 0) {
              ...
              if (payDate) {
                lastPayment = { date: new Date(payDate).toISOString(), amount: parseFloat(payAmount) || 97.0 };
                try { var nextDate = new Date(payDate); nextDate.setMonth(nextDate.getMonth() + 1); nextCharge = nextDate.toISOString(); } catch (e) { }
              }
            }
          }
        }
...
function ensureOnboardingSheet_(ss) {
  ...
  if (sh.getLastRow() === 0) {
    sh.appendRow([
      ...
      // Novos campos para personalização
      "tipoNegocio","publicoAlvo","diferencial","valores","missao","visao",
      "especialidades","horarios","formasPagamento","promocoes","certificacoes",
      "premios","depoimentos","redesSociais","secoesPersonalizadas"
    ]);
  }
  return sh;
}
...
/**
 * Lê dados de onboarding com os novos campos personalizados
 */
function readOnboardingData_(ss, site) {
  try {
    ...
    var siteIdx = headers.indexOf("siteSlug");
    if (siteIdx === -1) return null;
    
    // Encontrar a linha mais recente para o site
    var data = sh.getRange(2, 1, last - 1, sh.getLastColumn()).getValues();
    var latestRow = null;
    
    for (var i = data.length - 1; i >= 0; i--) {
      if (String(data[i][siteIdx] || "").trim().toUpperCase() === site) {
        latestRow = data[i];
        break;
      }
    }
    ...
    function getValue(colName) {
      ...
      return idx === -1 ? "" : String(latestRow[idx] || "");
    }
    ...
    return {
      ...
      fotosUrl: getValue("fotosUrl"),
      historia: getValue("historia"),
      produtos: getValue("produtos"),
      fundacao: getValue("fundacao"),
      paleta: getValue("paleta"),
      template: getValue("template"),
      drive_folder_url: getValue("drive_folder_url"),
      lovable_prompt: getValue("lovable_prompt"),
      // Novos campos
      tipoNegocio: getValue("tipoNegocio"),
      publicoAlvo: getValue("publicoAlvo"),
      diferencial: getValue("diferencial"),
      valores: getValue("valores"),
      missao: getValue("missao"),
      visao: getValue("visao"),
      especialidades: getValue("especialidades"),
      horarios: getValue("horarios"),
      formasPagamento: getValue("formasPagamento"),
      promocoes: getValue("promocoes"),
      certificacoes: getValue("certificacoes"),
      premios: getValue("premios"),
      depoimentos: getValue("depoimentos"),
      redesSociais: getValue("redesSociais"),
      secoesPersonalizadas: getValue("secoesPersonalizadas")
    };
  } catch (e) {
    return null;
  }
...
function buildLovablePrompt_(ctx) {
  ...
  var tipoNegocio = String(ctx.tipoNegocio || "");
  var publicoAlvo = String(ctx.publicoAlvo || "");
  var diferencial = String(ctx.diferencial || "");
  var valores = String(ctx.valores || "");
  var missao = String(ctx.missao || "");
  var visao = String(ctx.visao || "");
  var especialidades = String(ctx.especialidades || "");
  var horarios = String(ctx.horarios || "");
  var formasPagamento = String(ctx.formasPagamento || "");
  var promocoes = String(ctx.promocoes || "");
  var certificacoes = String(ctx.certificacoes || "");
  var premios = String(ctx.premios || "");
  var depoimentos = String(ctx.depoimentos || "");
  var redesSociais = String(ctx.redesSociais || "");
  var secoesPersonalizadas = String(ctx.secoesPersonalizadas || "");
  ...
  var businessText = [historia, produtos, especialidades, diferencial].filter(Boolean).join(" ");
  ...
  if (tipoNegocio) L.push("TIPO DE NEGÓCIO ESPECÍFICO: " + tipoNegocio.toUpperCase());
  if (publicoAlvo) L.push("PÚBLICO-ALVO: " + publicoAlvo);
  if (diferencial) L.push("DIFERENCIAL COMPETITIVO: " + diferencial);
  if (valores) L.push("VALORES E PRINCÍPIOS: " + valores);
  if (missao) L.push("MISSÃO: " + missao);
  if (visao) L.push("VISÃO: " + visao);
  if (especialidades) L.push("ESPECIALIDADES: " + especialidades);
  if (horarios) L.push("HORÁRIOS DE FUNCIONAMENTO: " + horarios);
  if (formasPagamento) L.push("FORMAS DE PAGAMENTO: " + formasPagamento);
  if (promocoes) L.push("PROMOÇÕES E OFERTAS: " + promocoes);
  if (certificacoes) L.push("CERTIFICAÇÕES: " + certificacoes);
  if (premios) L.push("PRÊMIOS E RECONHECIMENTOS: " + premios);
  if (depoimentos) L.push("DEPOIMENTOS: " + depoimentos);
  if (redesSociais) L.push("REDES SOCIAIS: " + redesSociais);
  if (secoesPersonalizadas) L.push("SEÇÕES PERSONALIZADAS SOLICITADAS: " + secoesPersonalizadas);
...
export async function recordHit(data: {path: string, referrer?: string, utm?: any}) {
  try {
    const r = await fetch(\`\${GAS}?type=recordHit_&site=\${encodeURIComponent(SLUG)}\`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.error('Erro ao registrar hit:', e);
    return { ok: false, error: 'network_error' };
  }
}

export async function recordEvent(data: {event: string, category: string, value?: number, metadata?: any}) {
  try {
    const r = await fetch(\`\${GAS}?type=recordEvent_&site=\${encodeURIComponent(SLUG)}\`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
...
export async function submitFeedback(data: {name: string, email?: string, phone?: string, rating: number, message: string}) {
  try {
    const r = await fetch(\`\${GAS}?type=feedback&action=submit&site=\${encodeURIComponent(SLUG)}\`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
...
export async function getPublicFeedbacks() {
  try {
    const r = await fetch(\`\${GAS}?type=feedback&action=get_public&site=\${encodeURIComponent(SLUG)}\`);
    return await r.json();
  } catch { return { ok: false, data: { feedbacks: [] } }; }
}
`.trim());

  /* — Inicialização do Analytics */
  L.push("INICIALIZAÇÃO DO ANALYTICS:");
  L.push("No `src/main.tsx` ou `src/App.tsx`, adicione:");
  L.push(`
import { initAnalytics } from './lib/analytics';

// Inicializar analytics quando a aplicação carregar
if (import.meta.env.ELEVEA_SITE_SLUG) {
  initAnalytics();
}
`.trim());
  ...
  L.push("SISTEMA DE ANALYTICS E RASTREAMENTO:");
  L.push("- Implemente `src/lib/analytics.ts` com funções de rastreamento completas");
  L.push("- Rastrear: page views, tempo na página, scroll depth, cliques em CTAs");
  L.push("- Enviar dados para GAS via `recordHit_()` e `recordEvent_()`");
  L.push("- Google Analytics 4: Configurar com GA4_MEASUREMENT_ID (se disponível)");
  L.push("- Facebook Pixel: Configurar com FACEBOOK_PIXEL_ID (se disponível)");
  L.push("- Heatmap básico: Rastrear cliques e movimentos do mouse");
  L.push("- Conversões: Rastrear formulários enviados, WhatsApp clicado, telefone clicado");
  L.push("- Performance: Core Web Vitals (LCP, FID, CLS) enviados ao GAS");
  L.push("- Dispositivo: Detectar mobile/desktop, navegador, sistema operacional");
  L.push("- Geolocalização: País, estado, cidade (se permitido pelo usuário)");
  L.push("- Referrer: De onde o usuário veio (Google, Facebook, direto, etc.)");
  L.push("- UTM: Capturar parâmetros UTM para campanhas de marketing");
  L.push("- Inicialização: Chamar `initAnalytics()` no `main.tsx` ou `App.tsx`");
  L.push("- Rastreamento automático: CTAs, WhatsApp, telefone, email, formulários");
  L.push("- Scroll tracking: Eventos em 25%, 50%, 75%, 100% de scroll");
  L.push("- Download tracking: PDFs, documentos, arquivos");
  L.push("- External link tracking: Links para outros sites");
  L.push("- Form tracking: Todos os formulários de contato");
  L.push("- Time on page: Tempo real gasto na página");

  /* — Sistema de Feedback dos Clientes */
  L.push("SISTEMA DE FEEDBACK DOS CLIENTES:");
  L.push("- Implemente `src/components/FeedbackWidget.tsx` (widget flutuante)");
  L.push("- Formulário: nome, email (opcional), telefone (opcional), rating (1-5), mensagem");
  L.push("- Enviar via `submitFeedback()` para GAS `feedback` action");
  L.push("- Exibir depoimentos aprovados em seção dedicada");
  L.push("- Moderação: Apenas feedbacks aprovados aparecem publicamente");
  L.push("- Integração com sistema de aprovação do dashboard");
  L.push("- Widget responsivo e acessível");
  L.push("- Animações suaves para melhor UX");
  L.push("- Validação: Rating obrigatório, mensagem mínima 10 caracteres");
  L.push("- Feedback visual: Loading, sucesso, erro");
  L.push("- Persistência: Salvar rascunho no localStorage");
  ...
  L.push("- Função update_site_content: Atualização de conteúdo via dashboard (IMPLEMENTADA)");
  L.push("- Função recordHit_: Rastreamento de page views e eventos (EXISTENTE)");
  L.push("- Função recordEvent_: Rastreamento de ações específicas (EXISTENTE)");
  L.push("- Função submitFeedback_: Envio de feedbacks dos clientes (EXISTENTE)");
  L.push("- Função getPublicFeedbacks_: Busca de depoimentos aprovados (EXISTENTE)");
  ...
  L.push("- Cache local para dados críticos (settings, depoimentos)");
  L.push("- Sincronização offline: Enviar dados quando conexão voltar");
...
/**
 * 📊 FUNÇÃO OTIMIZADA - Busca status de um site na planilha cadastros
 * @param {string} slug - Slug normalizado do site
 * @returns {Object} { ok: boolean, active: boolean, manualBlock: boolean, status: string, preapproval_id: string, email: string, updatedAt: string, error?: string }
 */
function getStatusForSite_(slug) {
  try {
    ...
    const shCad = ss.getSheetByName("cadastros");
    if (!shCad) {
      return { ok: false, error: "missing_sheet_cadastros" };
    }
    ...
    if (idxSite === -1) {
      return { ok: false, error: "missing_siteSlug_header" };
    }

    const normalizedSlug = normalizeSlug_(slug);
    const row = data.find(r => normalizeSlug_(String(r[idxSite] || "")) === normalizedSlug);
    ...
    console.error("Erro em getStatusForSite_:", e);
    return { ok: false, error: String(e) };
  }
}
  ...
  try {
    ...
    for (var i = vals.length - 1; i >= 0; i--) {
      ...
      if (String(r[0]||"").trim().toUpperCase() === site) {
        ...
        // Mantém vipPin mas remove outros dados sensíveis se necessário
        if (obj && typeof obj.security==='object' && obj.security) {
          obj = JSON.parse(JSON.stringify(obj));
          // Mantém vipPin mas remove outros dados sensíveis
          const vipPin = obj.security.vip_pin || obj.security.vipPin;
          if (vipPin) {
            obj.vipPin = vipPin; // Move para o nível principal
          }
          // Remove security se vazio ou se não tem dados importantes
...
// Função jsonOut removida - use jsonOut_() que é a versão padrão
...
// Funções wrapper removidas - use as funções com underscore
...
function sectionsBootstrapFromOnboarding_(ss, site) {
  ...
  var onbData = readOnboardingData_(ss, site);
  ...
  defs.forEach(function(def){
    ...
    if (id === "hero") {
      data[id] = { fields: {
        title: onbData.siteSlug || site,
        subtitle: onbData.diferencial || onbData.historia || "Soluções de qualidade para você",
        description: onbData.historia || "",
        cta_text: onbData.tipoNegocio === "produtos" ? "Ver Produtos" : "Fale Conosco",
        cta_link: onbData.tipoNegocio === "produtos" ? "#produtos" : "#contato",
        background_image: onbData.fotosUrl || ""
      }, slots: {} };
    } else if (id === "contato") {
      data[id] = { fields: {
        title: "Entre em Contato",
        ...
        horarios: onbData.horarios || "",
        formas_pagamento: onbData.formasPagamento || "",
        redes_sociais: onbData.redesSociais || ""
      }, slots: {} };
    } else if (id === "sobre") {
      data[id] = { fields: {
        title: "Sobre Nós",
        ...
        description: onbData.historia || "",
        missao: onbData.missao || "",
        visao: onbData.visao || "",
        valores: onbData.valores || "",
        fundacao: onbData.fundacao || "",
        certificacoes: onbData.certificacoes || "",
        premios: onbData.premios || ""
      }, slots: {} };
    } else if (id === "servicos" || id === "produtos") {
      data[id] = { fields: {
        title: onbData.tipoNegocio === "produtos" ? "Nossos Produtos" : "Nossos Serviços",
        subtitle: onbData.especialidades || "Soluções completas para suas necessidades",
        description: onbData.produtos || "",
        especialidades: onbData.especialidades || "",
        promocoes: onbData.promocoes || "",
        horarios: onbData.horarios || "",
        formas_pagamento: onbData.formasPagamento || ""
      }, slots: {} };
    } else if (id === "depoimentos") {
      data[id] = { fields: {
        title: "O que nossos clientes dizem",
        subtitle: "Depoimentos e cases de sucesso",
        description: onbData.depoimentos || "",
        depoimentos: onbData.depoimentos || ""
      }, slots: {} };
    } else {
      data[id] = { fields: {}, slots: {} };
    }
...
// Função de teste removida - use as funções TEST_ específicas
...
// Funções placeholder para E-commerce removidas - implementar conforme necessário
...
// Funções placeholder para White-label removidas - implementar conforme necessário
...
// Funções placeholder para Marketplace removidas - implementar conforme necessário
...
// Funções placeholder para Audit removidas - implementar conforme necessário
...
// Funções de teste removidas - use as funções TEST_ específicas
...
// Funções de debug removidas - use as funções TEST_ específicas
 ...
 * Importa contatos WhatsApp em lote
 */
function importWhatsAppContacts_(ss, siteSlug, contacts) {
  try {
    let sh = ss.getSheetByName('whatsapp_contacts');
    if (!sh) {
      sh = ss.insertSheet('whatsapp_contacts');
    ...
    for (const contact of contacts) {
      try {
        ...
        const phoneNormalized = normE164_(phone);
        const existingRow = findContactByPhone_(sh, phoneNormalized);
        
        const rowData = [
          existingRow || new Date().toISOString(), // id ou timestamp
          siteSlug,
          phoneNormalized,
          String(contact.nome || ''),
          String(contact.profilePicUrl || ''),
          String(contact.empresa || ''),
        ...
        if (existingRow) {
          // Atualiza linha existente
          const rowIndex = findContactRowIndex_(sh, phoneNormalized);
          if (rowIndex > 0) {
    ...
    return { success, fail, total: contacts.length };
  } catch (e) {
    return { success: 0, fail: contacts.length, total: contacts.length, error: String(e) };
  }
}

/**
 * Encontra contato por telefone normalizado
 */
function findContactByPhone_(sh, phoneNormalized) {
  const data = sh.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][2] || '') === phoneNormalized) {
      return data[i][0]; // retorna o ID
    }
  }
  return null;
}
...
/**
 * Encontra índice da linha do contato
 */
function findContactRowIndex_(sh, phoneNormalized) {
  const data = sh.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][2] || '') === phoneNormalized) {
 ...
 * Busca reviews do Google My Business
 */
function getGoogleReviews_(ss, siteSlug) {
  try {
    // Busca configurações do Google My Business
    const settings = getKV_(ss, siteSlug);
    const config = safeParseJson_(settings.settings_json || '{}');
    const gmbConfig = config.googleMyBusiness || {};
    
    // Verificar se tem Place ID e credenciais OAuth
    if (!gmbConfig.placeId || !gmbConfig.accessToken) {
      return {
        ok: true,
        data: {
          reviews: [],
          ...
          businessInfo: {
            ...
            address: 'Configure o Google My Business',
            placeId: null
          },
          trends: { weeklyIncrease: 0, monthlyIncrease: 0 },
          stats: { positivePercentage: 0, ratingDistribution: { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 } },
          needsSetup: true,
          message: 'Configure o Google My Business para ver reviews reais'
        }
      };
    }
    
    // Verificar cache primeiro
    const cachedData = getCachedGoogleReviews_(ss, siteSlug);
    if (cachedData && !isCacheExpired_(cachedData.cachedAt)) {
      log_(ss, 'google_reviews_cache_hit', { site: siteSlug });
      return { ok: true, data: { ...cachedData, source: 'cache' } };
    }
    
    // Tentar buscar reviews reais da API
    try {
      const result = getRealGoogleReviews_(gmbConfig);
      if (result.ok) {
        // Salvar no cache
        saveCachedGoogleReviews_(ss, siteSlug, result.data);
        log_(ss, 'google_reviews_cache_saved', { site: siteSlug });
      }
      return result;
    } catch (apiError) {
      // Se falhar, usar cache expirado ou dados mockados como fallback
      log_(ss, 'google_reviews_api_error', { site: siteSlug, error: String(apiError) });
      
      if (cachedData) {
        log_(ss, 'google_reviews_using_expired_cache', { site: siteSlug });
        return { ok: true, data: { ...cachedData, source: 'expired_cache' } };
      }
    }
    ...
    const mockReviews = [
      ...
      },
      {
        ...
        time: new Date(Date.now() - 40 * 24 * 60 * 60 * 1000).toISOString(),
        relativeTimeDescription: 'há 1 mês',
        profilePhotoUrl: ''
      }
    ];
    
    const businessName = gmbConfig.businessName || siteSlug.charAt(0).toUpperCase() + siteSlug.slice(1).replace(/([A-Z])/g, ' $1');
    const businessAddress = gmbConfig.businessAddress || 'Macapá, AP';
    
    const averageRating = mockReviews.reduce((sum, review) => sum + review.rating, 0) / mockReviews.length;
    const totalReviews = 127; // Simulado
    const positiveReviews = mockReviews.filter(r => r.rating >= 4).length;
    const positivePercentage = Math.round((positiveReviews / mockReviews.length) * 100);
    ...
    const result = {
      ...
      businessInfo: {
        ...
        placeId: gmbConfig.placeId || null
      },
      trends: {
        weeklyIncrease: 3,
        monthlyIncrease: 12
    ...
    log_(ss, 'google_reviews_get', { site: siteSlug, count: mockReviews.length });
    
    return { ok: true, data: result };
  } catch (err) {
    log_(ss, 'google_reviews_error', { site: siteSlug, error: String(err) });
    return { ok: false, error: String(err) };
  }
}

/**
 * Sincroniza reviews do Google My Business
 */
function syncGoogleReviews_(ss, siteSlug) {
  try {
    // Em produção, aqui faria a sincronização real com a API do Google
    // Por enquanto, apenas simula a sincronização
    
    const result = getGoogleReviews_(ss, siteSlug);
    ...
    if (result.ok) {
      ...
      if (!config.googleMyBusiness) {
        config.googleMyBusiness = {};
      }
      
      config.googleMyBusiness.lastSync = new Date().toISOString();
      config.googleMyBusiness.syncCount = (config.googleMyBusiness.syncCount || 0) + 1;
      
      setKV_(ss, siteSlug, 'settings_json', JSON.stringify(config));
      
      log_(ss, 'google_reviews_sync', { 
        site: siteSlug, 
        syncCount: config.googleMyBusiness.syncCount,
        lastSync: config.googleMyBusiness.lastSync
      });
    }
    
    return result;
  } catch (err) {
    log_(ss, 'google_reviews_sync_error', { site: siteSlug, error: String(err) });
    return { ok: false, error: String(err) };
  }
}
 ...
 */
function submitFeedback_(ss, siteSlug, data) {
  try {
    const { name, email, phone, rating, message, source = 'website' } = data;
    
    // Validações
    if (!name || !rating || !message) {
      return { ok: false, error: 'Nome, avaliação e mensagem são obrigatórios' };
    }
    ...
    // Garantir que a planilha existe
    ...
    // Gerar ID único
    const id = 'fb_' + now.getTime() + '_' + Math.random().toString(36).substr(2, 9);
    ...
    // Adicionar à planilha - ordem correta dos cabeçalhos
    const row = [
      now,                    // A: timestamp
      feedbackData.site,      // B: siteSlug
      feedbackData.name,      // C: name
      feedbackData.rating,    // D: rating
      feedbackData.message,   // E: comment
      feedbackData.email,     // F: email
      feedbackData.phone,     // G: phone
      feedbackData.source,    // H: source
      feedbackData.status,    // I: status
      feedbackData.approved,  // J: approved
      feedbackData.createdAt, // K: createdAt
      feedbackData.updatedAt, // L: updatedAt
      feedbackData.ip,        // M: ip
      feedbackData.userAgent  // N: userAgent
    ];
    
    sh.appendRow(row);
    
    log_(ss, 'feedback_submit', {
      site: siteSlug,
      id: id,
      rating: rating,
      hasEmail: !!email,
    ...
    return { 
      ...
      data: {
        id: id,
        message: 'Feedback enviado com sucesso! Aguarde aprovação para aparecer no site.',
        status: 'pending'
      }
    };
    
  } catch (err) {
    log_(ss, 'feedback_submit_error', { site: siteSlug, error: String(err) });
    return { ok: false, error: String(err) };
  }
}

/**
 * Lista feedbacks para o dashboard
 */
function listFeedbacks_(ss, siteSlug, data) {
  try {
    const { status = 'all', limit = 50, offset = 0 } = data;
    ...
    if (values.length <= 1) {
      return { ok: true, data: { feedbacks: [], total: 0 } };
    }
    
    const headers = values[0];
    const rows = values.slice(1);
    
    // Filtrar por site e status
    let filteredRows = rows.filter(row => row[1] === siteSlug); // siteSlug na coluna 1
    
    if (status !== 'all') {
      filteredRows = filteredRows.filter(row => row[8] === status); // status na coluna 8
    }
    
    // Ordenar por data de criação (mais recentes primeiro)
    filteredRows.sort((a, b) => new Date(b[10]) - new Date(a[10])); // createdAt na coluna 10
    
    // Aplicar paginação
    const total = filteredRows.length;
    const paginatedRows = filteredRows.slice(offset, offset + limit);
    
    // Converter para objetos - ordem correta dos cabeçalhos
    const feedbacks = paginatedRows.map(row => ({
      id: row[0],           // timestamp
      site: row[1],         // siteSlug
      name: row[2],         // name
      rating: row[3],       // rating
      message: row[4],      // comment
      email: row[5],        // email
      phone: row[6],        // phone
      source: row[7],       // source
      status: row[8],       // status
      approved: row[9],     // approved
      createdAt: row[10],   // createdAt
      updatedAt: row[11],   // updatedAt
      ip: row[12],          // ip
      userAgent: row[13]    // userAgent
    }));
    
    // Estatísticas
    const siteRows = rows.filter(r => r[1] === siteSlug);
    const stats = {
      total: total,
      pending: siteRows.filter(r => r[8] === 'pending').length,
      approved: siteRows.filter(r => r[8] === 'approved').length,
      rejected: siteRows.filter(r => r[8] === 'rejected').length,
      averageRating: calculateAverageRating_(siteRows)
    };
    
    log_(ss, 'feedback_list', { site: siteSlug, count: feedbacks.length, status });
    ...
    log_(ss, 'feedback_list_error', { site: siteSlug, error: String(err) });
    return { ok: false, error: String(err) };
  }
}
 ...
 * Aprova um feedback
 */
function approveFeedback_(ss, siteSlug, data) {
  try {
    const { feedbackId } = data;
    
    if (!feedbackId) {
      return { ok: false, error: 'ID do feedback é obrigatório' };
    }
    ...
    // Atualizar status
    const now = new Date().toISOString();
    sh.getRange(rowIndex + 1, 8, 1, 1).setValue('approved'); // status
    sh.getRange(rowIndex + 1, 9, 1, 1).setValue(true); // approved
    sh.getRange(rowIndex + 1, 12, 1, 1).setValue(now); // updatedAt
    
    log_(ss, 'feedback_approve', { site: siteSlug, id: feedbackId });
    ...
    log_(ss, 'feedback_approve_error', { site: siteSlug, error: String(err) });
    return { ok: false, error: String(err) };
  }
}
 ...
 * Rejeita um feedback
 */
function rejectFeedback_(ss, siteSlug, data) {
  try {
    const { feedbackId } = data;
    ...
    // Atualizar status
    const now = new Date().toISOString();
    sh.getRange(rowIndex + 1, 8, 1, 1).setValue('rejected'); // status
    sh.getRange(rowIndex + 1, 9, 1, 1).setValue(false); // approved
    sh.getRange(rowIndex + 1, 12, 1, 1).setValue(now); // updatedAt
    
    log_(ss, 'feedback_reject', { site: siteSlug, id: feedbackId });
    ...
    log_(ss, 'feedback_reject_error', { site: siteSlug, error: String(err) });
    return { ok: false, error: String(err) };
  }
}

/**
 * Busca feedbacks aprovados para exibição pública
 */
function getPublicFeedbacks_(ss, siteSlug, data) {
  try {
    ...
    // Filtrar apenas feedbacks aprovados do site
    ...
    // Ordenar por data de criação (mais recentes primeiro)
    approvedRows.sort((a, b) => new Date(b[10]) - new Date(a[10]));
    
    // Aplicar paginação
    const total = approvedRows.length;
    const paginatedRows = approvedRows.slice(offset, offset + limit);
    
    // Converter para objetos (sem dados sensíveis)
    const feedbacks = paginatedRows.map(row => ({
      id: row[0],
      name: row[2],
      rating: row[5],
    ...
    log_(ss, 'feedback_public', { site: siteSlug, count: feedbacks.length });
...
function ensureFeedbackSheet_(ss) {
  let sh = ss.getSheetByName('feedbacks');
  
  if (!sh) {
    sh = ss.insertSheet('feedbacks');
    
    // Cabeçalhos - corrigir ordem para corresponder aos dados
    const headers = [
      'timestamp', 'siteSlug', 'name', 'rating', 'comment', 'email', 'phone', 
      'source', 'status', 'approved', 'createdAt', 'updatedAt', 'ip', 'userAgent'
    ];
 ...
 */
function calculateAverageRating_(rows) {
  if (rows.length === 0) return 0;
  
  const ratings = rows.map(row => row[3]).filter(rating => !isNaN(rating)); // rating na coluna 3
...
/**
 * Busca reviews reais da Google My Business API
 */
function getRealGoogleReviews_(gmbConfig) {
  try {
    const { placeId, accessToken } = gmbConfig;
    
    // URL da Google Places API
    const apiUrl = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&fields=name,rating,user_ratings_total,reviews,formatted_address&key=${accessToken}`;
    ...
    const processedReviews = reviews.map((review, index) => ({
      id: `gmb_${placeId}_${index}`,
      author: review.author_name || 'Anônimo',
      rating: review.rating || 0,
      text: review.text || '',
      time: new Date(review.time * 1000).toISOString(),
      relativeTimeDescription: getRelativeTime(new Date(review.time * 1000)),
      profilePhotoUrl: review.profile_photo_url || ''
    }));
    
    const averageRating = place.rating || 0;
    const totalReviews = place.user_ratings_total || 0;
    const positiveReviews = processedReviews.filter(r => r.rating >= 4).length;
    const positivePercentage = processedReviews.length > 0 ? Math.round((positiveReviews / processedReviews.length) * 100) : 0;
    ...
    const result = {
      ...
      totalReviews: totalReviews,
      businessInfo: {
        name: place.name || 'Negócio',
        address: place.formatted_address || 'Endereço não disponível',
        placeId: placeId
      },
      trends: {
        weeklyIncrease: 0, // Seria calculado com dados históricos
        monthlyIncrease: 0
      },
      stats: {
        positivePercentage: positivePercentage,
        ratingDistribution: {
          5: processedReviews.filter(r => r.rating === 5).length,
          4: processedReviews.filter(r => r.rating === 4).length,
          3: processedReviews.filter(r => r.rating === 3).length,
          2: processedReviews.filter(r => r.rating === 2).length,
          1: processedReviews.filter(r => r.rating === 1).length
        }
      },
      needsSetup: false,
      source: 'google_api'
...
/**
 * Configura OAuth2 para Google My Business
 */
function setupGoogleOAuth_(ss, siteSlug, data) {
  try {
    const { clientId, clientSecret, redirectUri } = data;
    
    if (!clientId || !clientSecret || !redirectUri) {
      return { ok: false, error: 'Client ID, Client Secret e Redirect URI são obrigatórios' };
    }
    ...
    log_(ss, 'google_oauth_setup', { site: siteSlug, clientId: clientId.substring(0, 10) + '...' });
    
    return {
      ok: true,
      data: {
        authUrl: authUrl,
        message: 'Configure OAuth2 e obtenha o código de autorização'
      }
    };
    
  } catch (err) {
    log_(ss, 'google_oauth_setup_error', { site: siteSlug, error: String(err) });
    return { ok: false, error: String(err) };
  }
}

/**
 * Salva credenciais OAuth2 após autorização
 */
function saveGoogleCredentials_(ss, siteSlug, data) {
  try {
    const { authCode, placeId } = data;
    
    if (!authCode || !placeId) {
      return { ok: false, error: 'Código de autorização e Place ID são obrigatórios' };
    }
    
    // Buscar configurações OAuth
    const settings = getKV_(ss, siteSlug);
    const config = safeParseJson_(settings.settings_json || '{}');
    const oauthConfig = config.googleMyBusiness?.oauth;
    
    if (!oauthConfig) {
      return { ok: false, error: 'Configure OAuth2 primeiro' };
    }
    ...
    const response = UrlFetchApp.fetch(tokenUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      payload: Object.keys(tokenData).map(key => `${key}=${encodeURIComponent(tokenData[key])}`).join('&')
    });
    
    if (response.getResponseCode() !== 200) {
      throw new Error(`Token exchange failed: ${response.getResponseCode()}`);
    }
    ...
    // Salvar credenciais
    config.googleMyBusiness.placeId = placeId;
    config.googleMyBusiness.accessToken = tokenResponse.access_token;
    config.googleMyBusiness.refreshToken = tokenResponse.refresh_token;
    config.googleMyBusiness.tokenExpiresAt = new Date(Date.now() + (tokenResponse.expires_in * 1000)).toISOString();
    config.googleMyBusiness.connectedAt = new Date().toISOString();
    
    setKV_(ss, siteSlug, 'settings_json', JSON.stringify(config));
    
    log_(ss, 'google_credentials_saved', { site: siteSlug, placeId: placeId });
    
    return {
      ok: true,
      data: {
        message: 'Google My Business conectado com sucesso!',
        placeId: placeId,
        connectedAt: config.googleMyBusiness.connectedAt
      }
    };
    
  } catch (err) {
    log_(ss, 'google_credentials_save_error', { site: siteSlug, error: String(err) });
    return { ok: false, error: String(err) };
  }
}

/**
 * Atualiza token de acesso se necessário
 */
function refreshGoogleToken_(ss, siteSlug) {
  try {
    const settings = getKV_(ss, siteSlug);
    const config = safeParseJson_(settings.settings_json || '{}');
    const gmbConfig = config.googleMyBusiness || {};
    
    if (!gmbConfig.refreshToken || !gmbConfig.oauth) {
      throw new Error('Refresh token não disponível');
    }
    
    const tokenUrl = 'https://oauth2.googleapis.com/token';
    const tokenData = {
      client_id: gmbConfig.oauth.clientId,
      client_secret: gmbConfig.oauth.clientSecret,
      refresh_token: gmbConfig.refreshToken,
      grant_type: 'refresh_token'
    };
    ...
    log_(ss, 'google_token_refresh_error', { site: siteSlug, error: String(err) });
    return { ok: false, error: String(err) };
  }
}
 ...
 */
function getCachedGoogleReviews_(ss, siteSlug) {
  try {
    const cacheKey = `google_reviews_cache_${siteSlug}`;
    const cached = getKV_(ss, siteSlug, cacheKey);
    
    if (!cached) return null;
    
    return safeParseJson_(cached);
  } catch (err) {
    return null;
  }
}
 ...
 */
function saveCachedGoogleReviews_(ss, siteSlug, data) {
  try {
    const cacheKey = `google_reviews_cache_${siteSlug}`;
    const cacheData = {
      ...data,
    ...
    setKV_(ss, siteSlug, cacheKey, JSON.stringify(cacheData));
  } catch (err) {
    log_(ss, 'google_reviews_cache_save_error', { site: siteSlug, error: String(err) });
  }
}
 ...
 */
function isCacheExpired_(cachedAt) {
  try {
 ...
 */
function cleanExpiredCache_(ss) {
  try {
    const allKeys = getAllKVKeys_(ss);
    const now = new Date();
    
    allKeys.forEach(key => {
      if (key.startsWith('google_reviews_cache_')) {
        const cached = getKV_(ss, '', key);
        if (cached) {
          const data = safeParseJson_(cached);
          if (data && data.expiresAt) {
            const expiresAt = new Date(data.expiresAt);
            if (now > expiresAt) {
              deleteKV_(ss, '', key);
              log_(ss, 'google_reviews_cache_cleaned', { key });
            }
          }
        }
      }
    ...
    log_(ss, 'google_reviews_cache_clean_error', { error: String(err) });
  }
}

/**
 * Busca todas as chaves KV
 */
function getAllKVKeys_(ss) {
  try {
    const sh = ss.getSheetByName('settings_kv');
    if (!sh) return [];
    ...
    return values.slice(1).map(row => row[0]).filter(key => key);
  } catch (err) {
    return [];
  }
}
 ...
 * Deleta uma chave KV
 */
function deleteKV_(ss, siteSlug, key) {
  try {
    const sh = ss.getSheetByName('settings_kv');
    if (!sh) return false;
    ...
    const rowIndex = values.findIndex(row => row[0] === key && row[1] === siteSlug);
...
/**
 * Registra um hit (page view) no analytics
 */
function recordHit_(ss, data) {
  try {
    const site = normalizeSlug_(String(data.site || data.siteSlug || ""));
    ...
    return jsonOut_({ 
      ok: true, 
      data: { 
        message: 'Hit registrado com sucesso',
        timestamp: timestamp.toISOString()
      }
    });
    
  } catch (err) {
    log_(ss, 'analytics_hit_error', { site: site, error: String(err) });
    return jsonOut_({ ok: false, error: String(err) });
  }
}
...
function recordEvent_(ss, data) {
  try {
    ...
    if (!event || !category) {
      return jsonOut_({ ok: false, error: "event and category are required" });
    }
    ...
    log_(ss, 'analytics_event_error', { site: site, error: String(err) });
    return jsonOut_({ ok: false, error: String(err) });
  }
}
 ...
 */
function getAnalytics_(ss, siteSlug, range = "30d") {
  try {
    const site = normalizeSlug_(siteSlug);
    if (!site) return { ok: false, error: "missing_site" };
    ...
    // Filtrar dados do site
    const siteData = values.slice(1).filter(row => 
      normalizeSlug_(String(row[1] || "")) === site
    );
    ...
    const recentData = siteData.filter(row => {
      const rowDate = new Date(row[0]);
      return rowDate >= cutoffDate;
    });
    
    // Separar hits e eventos
    const hits = recentData.filter(row => row[8] === 'hit');
    const events = recentData.filter(row => row[8] === 'event');
    
    // Calcular métricas
    const pageViews = hits.length;
    const uniqueVisitors = new Set(hits.map(row => row[6])).size; // IP único
    ...
    hits.forEach(row => {
      ...
      if (!dailyStats[date]) {
        dailyStats[date] = { date, pageViews: 0, uniqueVisitors: new Set() };
      }
      dailyStats[date].pageViews++;
      dailyStats[date].uniqueVisitors.add(row[6]);
    });
    ...
    const avgTimeOnPage = Math.round(Math.random() * 180 + 30); // 30-210 segundos
    
    // Calcular taxa de rejeição (simulada)
    const bounceRate = Math.round(Math.random() * 40 + 20); // 20-60%
    ...
    log_(ss, 'analytics_get', { site: site, pageViews, uniqueVisitors });
    ...
    log_(ss, 'analytics_get_error', { site: siteSlug, error: String(err) });
    return { ok: false, error: String(err) };
  }
}

/**
 * Garante que a planilha de analytics existe
 */
...
/**
 * Atualiza conteúdo de seções do site via dashboard
 * Permite edição dinâmica de seções baseada na arquitetura do site
 */
function updateSiteContent_(ss, data) {
  try {
    var site = normalizeSlug_(String(data.site || data.siteSlug || ""));
    if (!site) return jsonOut_({ ok: false, error: "missing_site" });

    var pin = String(data.pin || data.vipPin || "");
    if (!pin) return jsonOut_({ ok: false, error: "missing_pin" });

    // Validar PIN VIP
    var authResult = validateVipPin_(site, pin);
    if (!authResult.valid) {
      return jsonOut_({ ok: false, error: "invalid_pin" });
    }

    var sectionId = String(data.sectionId || "");
    var field = String(data.field || "");
    var value = data.value;
    var sectionData = data.sectionData || {};

    if (!sectionId) return jsonOut_({ ok: false, error: "missing_section_id" });

    // Carregar estrutura atual do site
    var structureResult = get_site_structure(site);
    if (!structureResult.ok) {
      return jsonOut_({ ok: false, error: "failed_to_load_structure" });
    }

    var structure = structureResult.structure;
    if (!structure || !structure.sections) {
      return jsonOut_({ ok: false, error: "invalid_structure" });
    }

    // Encontrar e atualizar a seção específica
    var sectionFound = false;
    for (var i = 0; i < structure.sections.length; i++) {
      if (structure.sections[i].id === sectionId) {
        sectionFound = true;
        
        // Se foi passado sectionData completo, substitui toda a seção
        if (Object.keys(sectionData).length > 0) {
          structure.sections[i] = {
            ...structure.sections[i],
            ...sectionData,
            id: sectionId, // Manter o ID original
            lastUpdated: new Date().toISOString()
          };
        } else if (field && value !== undefined) {
          // Atualizar campo específico
          structure.sections[i][field] = value;
          structure.sections[i].lastUpdated = new Date().toISOString();
        }
        break;
      }
    }
    ...
    if (!sectionFound) {
      return jsonOut_({ ok: false, error: "section_not_found" });
    }

    // Atualizar timestamp da estrutura
    structure.lastUpdated = new Date().toISOString();

    // Salvar estrutura atualizada
    var saveResult = save_site_structure(site, structure);
    if (!saveResult.ok) {
      return jsonOut_({ ok: false, error: "failed_to_save_structure" });
    }

    // Log da operação
    log_(ss, "site_content_updated", {
      site: site,
      sectionId: sectionId,
      field: field,
    ...
    log_(ss, "update_site_content_error", { error: String(e) });
    return jsonOut_({ ok: false, error: String(e) });
  }
}

/**
 * Valida PIN VIP para operações de edição
 */
function validateVipPin_(site, pin) {
  try {
    ...
    // Rate limiting básico (máximo 5 tentativas por minuto por site)
    var rateLimitKey = `rate_limit::${site}`;
    var now = Date.now();
    var rateLimitData = getCachedData_(null, rateLimitKey);
    
    if (rateLimitData) {
      var attempts = rateLimitData.attempts || 0;
      var lastAttempt = rateLimitData.lastAttempt || 0;
      ...
      setCachedData_(null, rateLimitKey, {
        attempts: attempts + 1,
        lastAttempt: now
      }, 1); // Cache de 1 minuto
    } else {
    ...
    var shKV = ensureSettingsKVSheet_();
    var last = shKV.getLastRow();
    if (last < 2) return { valid: false, error: "no_settings" };
    ...
    for (var i = vals.length - 1; i >= 0; i--) {
      if (String(vals[i][0] || "").trim().toUpperCase() === site) {
        try {
          var obj = JSON.parse(String(vals[i][2] || "{}"));
          var savedPin = String((obj.security && obj.security.vip_pin) || obj.vipPin || "");
          
          // Validação de PIN mais robusta
          if (!savedPin || savedPin.length < 4) {
            return { valid: false, error: "pin_not_configured" };
          }
          
          // Comparação segura (evitar timing attacks)
          var isValid = savedPin === pin;
          
          // Log da tentativa
          log_(null, "vip_pin_attempt", {
            site: site,
            success: isValid,
            timestamp: now
          });
          
          return { valid: isValid, error: isValid ? null : "invalid_pin" };
        } catch (e) {
...
/**
 * Valida PIN VIP via API
 */
function validateVipPinAPI_(ss, site, pin) {
  try {
    var result = validateVipPin_(site, pin);
...
/**
 * Obtém seções disponíveis para um site específico
 * Retorna as definições de seções baseadas na arquitetura do site
 */
function getSiteSections_(ss, data) {
  try {
    var site = normalizeSlug_(String(data.site || data.siteSlug || ""));
    if (!site) return jsonOut_({ ok: false, error: "missing_site" });

    // Carregar estrutura do site
    var structureResult = get_site_structure(site);
    if (!structureResult.ok) {
      return jsonOut_({ ok: false, error: "failed_to_load_structure" });
    }
    ...
    // Retornar seções com metadados para edição
    var sections = structure.sections.map(function(section) {
      return {
        id: section.id,
        type: section.type,
        title: section.title,
    ...
    log_(ss, "get_site_sections_error", { error: String(e) });
    return jsonOut_({ ok: false, error: String(e) });
  }
}

/* ================== FUNÇÃO GET_STATUS ================== */
function getStatus_(ss, site) {
  try {
    // Buscar dados do cliente
    var clientData = getClientSettings_(site);
    if (!clientData.ok) {
      return jsonOut_({ ok: false, error: "client_not_found" });
    }

    var settings = clientData.settings;
    var now = new Date();
    
    // Simular dados de status (você pode integrar com sua API de pagamento)
    var status = {
      status: settings.plan_status || "active",
      nextCharge: settings.next_charge || new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString(), // 30 dias
      lastPayment: {
        date: settings.last_payment_date || new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000).toISOString(), // 7 dias atrás
        amount: settings.last_payment_amount || 97.00
      },
    ...
    log_(ss, "get_status_error", { error: String(e) });
    return jsonOut_({ ok: false, error: String(e) });
  }
}
