...
export const handler = async (event) => {
  try {
    ...
    // Primeiro tenta auth_status (com suporte a PIN VIP)
    const pin = String(qs.pin || qs.vipPin || "").trim();
    
    let r;
    if (pin) {
      // Se tem PIN, usa auth_status que valida VIP
      r = await postToGas({
        action: "auth_status",
        site: siteSlug,
      ...
      // Se não tem PIN, usa client_billing
      r = await postToGas({
        type: "client_billing",
        email: String(claims.email || "").toLowerCase(),
        siteSlug,
    ...
    const normalized = {
      ...
      vip: d.vip || (d.plan && d.plan.toLowerCase().includes("vip")) || false,
      nextPayment: d.nextPayment || d.next_charge || d.nextCharge || null,
