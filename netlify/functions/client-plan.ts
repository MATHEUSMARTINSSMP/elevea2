...
export const handler: Handler = async (event) => {
  try {
    ...
    // Se tem PIN, usa auth-status diretamente; senão usa client-plan normal
    const pin = String(qs.pin || qs.vipPin || "").trim();
    ...
    let url;
    if (pin) {
      // Com PIN, usa auth-status que valida VIP
      url = `${base}/.netlify/functions/auth-status?site=${encodeURIComponent(site)}&pin=${encodeURIComponent(pin)}`;
    } else {
      // Sem PIN, usa auth-status normal
      url = `${base}/.netlify/functions/auth-status?site=${encodeURIComponent(site)}`;
    }
